function [alignment_matrix, initial_permutation, ACC_Pi, Loss_obj, Loss_pi] = tensor_align_f1_chemCMTF(Z, MaxMostOuterIter, thresholdResObj, str_factor, str_pi, k_cluster, guessRank, truePermu, str_row, str_col, str_entropy)
% Z: the input CMTF data
...
while(iter <= MaxMostOuterIter)
    ...
    for i = 1:mode1_size
        for j = 1:mode1_size
            if truePermu(i,j)==1 && alignment_matrix(i,j)>=0.1
                acc_pi = acc_pi+1;
            end
        end
    end
